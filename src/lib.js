...
var zr = zrender.init(divContainer,null,{renderer:'canvas',devicePixelRatio:2});
var domNode = document.getElementById("addNode"),domLine = document.getElementById("addLine"),
...
zr.on('click',function(){
    ...
    currentNode.attr({style:{stroke:SHAPE_LINE_COLOR}});
    currentNode = null;
});
...
domNode.addEventListener('click',function(){
    var txt = document.getElementById('nodeTxt').value,shape = createShape(txt);
    nodeCache.push(shape);
    shape.on('click',function(ev){
        this.attr({style:{stroke:ACTIVE_COLOR}});
        currentNode = this;
        checkedNode = true;
    });
    zr.add(shape);
